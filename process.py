...
# 消除离群点，保存最后的输出
def remove_outlier_main(jaw, pcd_points, labels, instances_labels, save_path):
    ...
    with open('/output/dental-labels.json', 'w') as f_obj:
        f_obj.write(b)
    f_obj.close()
...
# 将三角网格数据上采样回原始点云数据
def mesh_to_points_main(jaw, pcd_points, center_points, labels, save_path):
    points = pcd_points.copy()
    # 下采样
    ...
    remove_outlier_main(jaw, pcd_points, upsample_label, instances_labels, save_path)
...
def main(input_dir, save_dir, model_path):
    ...
    model.eval()
    with torch.no_grad():
        pcd_points, jaw = obj2pcd(input_dir)
        ...
        mesh_to_points_main(jaw, pcd_points, center_points, predicted_labels_3, save_dir)
